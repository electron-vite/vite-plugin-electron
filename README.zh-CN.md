...
> [!important]
> **`v1.0.0` 行为变更**：`notBundle()` 现在会在 config 阶段根据 `package.json` 一次性配置 `build.rolldownOptions.external`（Vite < 8 为 `build.rollupOptions.external`），不再在每次 `resolveId` 时校验每个 import 是否可被 `require()` 加载。
