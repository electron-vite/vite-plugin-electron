...
/** Named Vite environments used for the Electron main process and preload scripts. */
  ...
  const plugins: Plugin[] = [
    {
      ...
      config(userConfig, { command }) {
        ...
        const environments: Record<string, EnvironmentOptions> = {
          [ENVIRONMENT.main]: makeMainEnv(isDev),
        }
        const preloadEnv = makePreloadEnv(isDev)
        if (preloadEnv) {
          environments[ENVIRONMENT.preload] = preloadEnv
        }
      ...
      configureServer(server) {
        ...
        server.httpServer?.once('listening', async () => {
          ...
          const builder = await createBuilder({
            ...
            plugins: [
              {
                ...
                closeBundle() {
                  ...
                  if (envName !== ENVIRONMENT.main && envName !== ENVIRONMENT.preload) return
            ...
            environments: {
              [ENVIRONMENT.main]: makeMainEnv(true),
              ...(options.preload ? { [ENVIRONMENT.preload]: makePreloadEnv(true)! } : {}),
            },
          })
